...
import type { StartOpts, StartResult, Summary, FileRow, FileDone, Progress, HealthStatus, CachedFile, ExportedEvent, EventRow } from "./types";
...
/** Re-run failed files for an existing session without re-picking the folder.
 * Pass `paths` to retry only those files; omit to retry every failed file. */
export const retryFailed = (opts: StartOpts, sessionId: number, paths?: string[]) =>
  invoke<StartResult>("retry_failed", { opts, sessionId, paths: paths ?? null });
...
export const onFileDone = (cb: (f: FileDone) => void): Promise<UnlistenFn> =>
  listen<FileDone>("batch://file_done", (e) => cb(e.payload));
